...
Group::Group(int N)
{
  cout << "Initializing group SU(" << N << ") ..." << endl;
  Nc = N;
  ...
  tA = new Matrix*[Nc*Nc-1]; // 3 generators for SU(2), 8 for SU(3) in the adjoint representation
  for (int i=0; i<Nc*Nc-1; i++)
    {
...
//       it[2]->set(1,0,0.);                      it[2]->set(1,1,complex<double>(0.,-0.5));
    ...
    {
      cout << "Ok, this program only works for SU(2) and SU(3). You gave me SU(" << Nc << "). I can't do that. Write your own program. Exiting." << endl;
      exit(1);
    }
